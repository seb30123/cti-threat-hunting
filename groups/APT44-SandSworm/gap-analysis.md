...
> MITRE G0034 liste ~80 techniques Enterprise toutes campagnes confondues. Un taux de 55% sur cette analyse est élevé grâce aux sources 2024–2026 — bien au-dessus d'une première analyse standard.
...
| ID | Technique | Ma source | MITRE G0034 |
|----|-----------|-----------|-------------|
...
## Techniques MITRE importantes manquées
...
1. **CERT Polska [R23]** est la source la plus technique et la plus récente pour la chaîne IT→OT (déc. 2025). Contient des TTPs uniques non encore dans MITRE officiel.
2. **Le Diamond Ticket (T1558)** est distinct de T1558.001 (Golden Ticket) — ne pas confondre. APT44 utilise la variante qui modifie un TGT légitime (plus difficile à détecter).
3. **La famille wiper est itérative** : ZEROLOT → Sting → ZOV → DynoWiper → LazyWiper. Chaque variante = même T1485 mais avec détails d'implémentation différents.
4. **ATT&CK for ICS est indispensable** pour analyser APT44 correctement — c'est le seul groupe avec des techniques ICS documentées in-the-wild (T0835, T0839).
5. **T1669 (Neighbor Wireless)** est une technique tellement récente qu'elle n'existait pas dans ATT&CK avant novembre 2024 — preuve que le mapping doit suivre les sources récentes, pas seulement MITRE.

---

## Actions pour la v2

- [ ] Lire CERT Polska Energy Sector Incident Report 2025 complet (cert.pl)
- [ ] Ajouter T1566.001 avec sources CERT-UA récentes
- [ ] Compléter les techniques ICS depuis Dragos ICS analysis (Industroyer)
- [ ] Analyser campagnes MITRE C0034 (NotPetya) et C0051 (Nearest Neighbor) séparément
- [ ] Documenter techniques BadPilot subgroup [R10] en détail
- [ ] Passer de 48 à 60+ techniques documentées
